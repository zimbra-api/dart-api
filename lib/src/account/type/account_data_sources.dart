...
class AccountDataSources {
  ...
  factory AccountDataSources.fromMap(
    Map<String, dynamic> data,
  ) =>
      AccountDataSources(
        imapDataSources: (data['imap'] is Iterable)
            ? (data['imap'] as Iterable)
                .map<AccountImapDataSource>(
                  (imap) => AccountImapDataSource.fromMap(imap),
                )
                .toList(growable: false)
            : const [],
        pop3DataSources: (data['pop3'] is Iterable)
            ? (data['pop3'] as Iterable)
                .map<AccountPop3DataSource>(
                  (pop3) => AccountPop3DataSource.fromMap(pop3),
                )
                .toList(growable: false)
            : const [],
        caldavDataSources: (data['caldav'] is Iterable)
            ? (data['caldav'] as Iterable)
                .map<AccountCaldavDataSource>(
                  (caldav) => AccountCaldavDataSource.fromMap(caldav),
                )
                .toList(growable: false)
            : const [],
        yabDataSources: (data['yab'] is Iterable)
            ? (data['yab'] as Iterable)
                .map<AccountYabDataSource>(
                  (yab) => AccountYabDataSource.fromMap(yab),
                )
                .toList(growable: false)
            : const [],
        rssDataSources: (data['rss'] is Iterable)
            ? (data['rss'] as Iterable)
                .map<AccountRssDataSource>(
                  (rss) => AccountRssDataSource.fromMap(rss),
                )
                .toList(growable: false)
            : const [],
        galDataSources: (data['gal'] is Iterable)
            ? (data['gal'] as Iterable)
                .map<AccountGalDataSource>(
                  (gal) => AccountGalDataSource.fromMap(gal),
                )
                .toList(growable: false)
            : const [],
        calDataSources: (data['cal'] is Iterable)
            ? (data['cal'] as Iterable)
                .map<AccountCalDataSource>(
                  (cal) => AccountCalDataSource.fromMap(cal),
                )
                .toList(growable: false)
            : const [],
        unknownDataSources: (data['unknown'] is Iterable)
            ? (data['unknown'] as Iterable)
                .map<AccountUnknownDataSource>(
                  (unknown) => AccountUnknownDataSource.fromMap(unknown),
                )
                .toList(growable: false)
            : const [],
      );
